 ...
 *
 * REGISTROS:
 *   - 0x0A → trigger de adquisición (escribir 0x30)
 *   - 0x30 → registro de estado (bit 3 = Sco, 0 = listo)
 ...
 *   0x6D — dirección estándar del lote usado en Aquantia.
 *   Si un sensor responde en 0x7F (lote alternativo), cambiar PRESSURE_SENSOR_I2C_ADDR.
 */
...
/** Dirección I2C del sensor (7-bit). Hardware Aquantia: 0x6D. */
#define PRESSURE_SENSOR_I2C_ADDR   0x6D

/**
 * Presión fondo de escala en kPa (rango del sensor instalado en Aquantia).
 *   0-4 bar (riego doméstico) → 400 kPa  ← valor actual
 *   0-10 bar                  → 1000 kPa
 *   0-40 bar                  → 4000 kPa
 */
#define PRESSURE_SENSOR_FULLSCALE  400.0f   // kPa — sensor 0-4 bar Aquantia
...
#define PSEN_REG_TRIGGER    0x0A   // Escribir 0x30 aquí para iniciar adquisición
#define PSEN_REG_STATUS     0x30   // Leer aquí; bit3 (Sco) = 1 → ocupado
...
#define PSEN_CMD_START_ACQ  0x30   // Comando de inicio de adquisición
 ...
 *
 * Flujo interno:
 *   1. Escribe 0x30 en reg 0x0A → inicia adquisición
